frappe.listview_settings['Attendance'] = {
				...
				primary_action(data) {
					if (cur_dialog.no_unmarked_days_left) {
						...
						frappe.confirm(__('Mark attendance as {0} for {1} on selected dates?', [data.status, data.month]), () => {
							frappe.call({
								method: "attendance.attendance.doctype.attendance.attendance.mark_bulk_attendance",
								args: {
									data: data
								},
								callback: function (r) {
	...
	get_multi_select_options: function(employee, month) {
		return new Promise(resolve => {
			frappe.call({
				method: 'attendance.attendance.doctype.attendance.attendance.get_employee_unmarked_days',
				async: false,
				args: {
					employee: employee,
					month: month,
